...
void HariMain(void)
{
	...
	/* 壒斣崋偲廃攇悢(mHz)偺懳墳昞 */
	/* 偨偲偊偽丄O4A偼440Hz側偺偱丄440000 */
	/* 僆僋僞乕僽16偺A偼1802240Hz側偺偱丄1802240000 */
	/* 埲壓偼僆僋僞乕僽16偺儕僗僩(C乣B) */
...
file_error:
	...
	/* 僂傿儞僪僂偺弨旛 */
	win = api_openwin(winbuf, 256, 112, -1, "MML蜂鸣播放器");
	...
	r = txtbuf;
	i = 0; /* 捠忢儌乕僪 */
	for (p = txtbuf; *p != 0; p++) {	/* 張棟傪娙扨偵偡傞偨傔偵僐儊儞僩傗嬻敀傪徚偡 */
		if (i == 0 && *p > ' ') {	/* 僗儁乕僗傗夵峴僐乕僪側偳偱偼側偄 */
			if (*p == '/') {
				if (p[1] == '*') {
					i = 1;
				} else if (p[1] == '/') {
		...
		} else if (i == 1 && *p == '*' && p[1] == '/') {	/* 僽儘僢僋僐儊儞僩 */
			p++;
			i = 0;
		} else if (i == 2 && *p == 0x0a) {	/* 峴僐儊儞僩 */
			i = 0;
		} else if (i == 3) {	/* 暥帤楍 */
			*r = *p;
			r++;
	...
	*r = 0;
	...
	for (;;) {
		if (('A' <= *p && *p <= 'G') || *p == 'R') {	/* 壒晞丒媥晞 */
			...
			o = strtol(p + 1, &p, 10);
		} else if (*p == 'Q') {	/* Q僷儔儊乕僞巜掕 */
			q = strtol(p + 1, &p, 10);
		} else if (*p == 'L') { /* 僨僼僅儖僩壒挿巜掕 */ 
			l = strtol(p + 1, &p, 10);
			if (l == 0) {
				goto syntax_error;
			}
