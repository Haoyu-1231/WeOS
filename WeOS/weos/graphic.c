...
void init_startscreen8(char *vram, int x, int y)
{
	boxfill8(vram, x, 137, 0, 0, x, y);
	return;
}
...
void init_screen8(char *vram, int x, int y)
{
	...
	boxfill8(vram, x, 137, 0, 0, x - 1, y - 29);
	...
	boxfill8(vram, x, COL8_848484, x - 150, y - 24, x - 4, y - 24);
	boxfill8(vram, x, COL8_848484, x - 150, y - 23, x - 150, y - 4);
	boxfill8(vram, x, COL8_FFFFFF, x - 150, y - 3, x - 4, y - 3);
	...
	if (task->langmode == 2) {
		for (; *s != 0x00; s++) {
			if (task->langbyte1 == 0) {
				...
				unsigned char right[16] = {0};
				int i;
				for (i = 0; i <= 15; i++) {
					left[i] = font[i * 2];
					right[i] = font[i * 2 + 1];
				}
				putfont8(vram, xsize, x - 8, y, c, left);
				putfont8(vram, xsize, x    , y, c, right);
			}
			x += 8;
		}
	}
...
int read_picture(int *fat, char *vram, int x, int y)
{
	...
	finfo = file_search("weos.jpg", (struct FILEINFO *) (ADR_DISKIMG + 0x002600), 224);
	if (finfo == 0) {
		return -1;
	}
	fsize = finfo->size;
