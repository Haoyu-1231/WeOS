...
static int tek_decmain5(int *work, UCHAR *src, int osiz, UCHAR *q, int lc, int pb, int lp, int flags)
{
	...
	if (stk) {
		...
		rd->rmsk |= -1;
		rd->bm[1].t = 5; rd->bm[1].m = 3;
		rd->bm[2].t = 9; rd->bm[2].m = 2;
		if (flags & 0x40) {
		...
		if (flags == -2) {
			...
			pt[ 1] =  8 + ((flags & 0x04) != 0);
			pt[ 5] = 12 + ((flags & 0x08) != 0);
			pt[ 9] = 16 + ((flags & 0x10) != 0);
			pt[11] = 18 + ((flags & 0x20) != 0);
		}
		for (i = 0; i < 14; i++)
	...
	*q++ = tek_rdget1(rd, prb->lit, lit0cntmsk, 1, &rd->bm[24]) & 0xff;
	...
	while (pos < osiz) {
		...
		if (tek_rdget1(rd, &prb->pb[s_pos].st[s].mch, 0x71, 0, rd->ptbm[s > 0]) ^ stk) {
			...
			pmch |= 1;
			if (tek_rdget1(rd, &prb->st[s].rep, 0x71, 0, rd->ptbm[13]) ^ stk) {
				rep[3] = rep[2];
				rep[2] = rep[1];
				rep[1] = rep[0];
				...
				if (tek_rdget1(rd, &prb->st[s].repg0, 0x71, 0, rd->ptbm[13]) ^ stk) {
					i |= -1;
					...
					if (tek_rdget1(rd, &prb->st[s].repg1, 0x71, 0, rd->ptbm[13]) ^ stk)
						i = rep[1];
					else {
						if (tek_rdget1(rd, &prb->st[s].repg2, 0x71, 0, rd->ptbm[13]) ^ stk)
							i = rep[2];
						else {
							if (stk == 0) {
